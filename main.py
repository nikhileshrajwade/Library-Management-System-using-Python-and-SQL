...
database = sqlite3.connect('library_management.db', check_same_thread=False)

# +++++ LOAD DATA +++++
library = pd.read_sql("SELECT * FROM library_books", database)
user = pd.read_sql("SELECT * FROM User_data", database)
admin = pd.read_sql("SELECT * FROM Admin_data", database)
removed_user = pd.read_sql("SELECT * FROM removed_user", database)

# +++++ SAVE FUNCTIONS +++++
def save_user():
    user.to_sql("User_data", database, if_exists="replace", index=False)

def save_library():
    library.to_sql("library_books", database, if_exists="replace", index=False)

def save_removed_user():
    removed_user.to_sql("removed_user", database, if_exists="replace", index=False)
...
# +++++ USER FINE +++++
def user_fine():
    # Added errors='coerce' to safely handle "NaT" strings from SQLite
    user["Issue_Date_1"] = pd.to_datetime(user["Issue_Date_1"], dayfirst=True, errors='coerce').dt.normalize()
    user["Issue_Date_2"] = pd.to_datetime(user["Issue_Date_2"], dayfirst=True, errors='coerce').dt.normalize()
    user["Issue_Date_3"] = pd.to_datetime(user["Issue_Date_3"], dayfirst=True, errors='coerce').dt.normalize()
...
def book_reissue():
    ...
    for i in range(3):
        ...
        if pd.notna(user.loc[user_index, str_name]) and (b_name == user.loc[user_index, str_name]):
...
def userID():
    ...
    # Only wrap the integer input in the try-except
    try:
        global User_ID
    ...
    if User_ID in ex_user:
        ...
        if pwd == U_Pass_All[np.where(U_ID_All == User_ID)[0][0]]:
            print("------")
            login_user() # This is now safely outside the error handling!
        else:
